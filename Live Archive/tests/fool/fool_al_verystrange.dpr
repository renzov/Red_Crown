...
{solution with alpha-beta pruning:
 NOTE: the order of moves is one of the most stupid}
const MaxC=6;
      ASize=1 shl 24;
      Suits='SCDH';
      Values='6789TJQKA';
...
begin
  ...
  for i:=0 to n-1 do 
    for j:=i+1 to n-1 do
      if (V[j]<V[i]) or ((S[j]<>TR) and (S[i]=TR)) then begin
        t:=V[i]; V[i]:=V[j]; V[j]:=t;
        c:=S[i]; S[i]:=S[j]; S[j]:=c;
        t1:=(st shr (i+i)) and 3;
        t2:=(st shr (j+j)) and 3;
        st:=st and not (3 shl (i+i)) and not (3 shl (j+j));
        st:=st or (t2 shl (i+i)) or (t1 shl (j+j));
      end;

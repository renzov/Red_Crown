...
public class cactus_petr_wa {
	...
	static class Cycle {
		...
		public int solve(Vertex root, int partSize) {
			...
			while (pos < numbers.length) {
				int last = pos;
				while (last < n && numbers[last + 1].number == numbers[last].number + partSize)
					++last;
	...
	public static void main(String[] args) {
		cactus_petr_wa solver = new cactus_petr_wa();
		solver.run();
		solver.out.close();
	}

	public cactus_petr_wa() {

...
void bin_search( int node, int L, int R, const int &y ){
	int M;
	while ( L < R ){
		M = ( L + R ) >> 1;
		if ( seg[left(node)] > y ){
	...
	assert( L == R );
	pii ans = pii( H[L], *S[L].upper_bound( y ) );
	res = min( res, ans );
}

void find_point( const int &node, const int &L, const int &R, const int &l, const int &r, const int &y ){
	if ( L == l && r == R ){
		if ( seg[node] > y ) bin_search( node, L, R, y );
		return;
	}
	int M = ( L + R ) >> 1;
	if ( r <= M ) return find_point( left(node), L, M, l, r, y );
		...
		find_point( left(node), L, M, l, M, y );
		find_point( right(node), M + 1, R, M + 1, r, y );
	}
}
...
int main(){
	...
	for ( int i=0; i < N; ++i ){
		if ( Q[i] == 0 ){
			...
			res = noAns;
			find_point( 1, 0, M - 1, idx, M - 1, Y[i] );
